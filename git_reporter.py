...
def find_git_repositories(base_path):
    """무거운 폴더를 스킵하고, .git 발견 시 하위 탐색을 중단하여 스캔 속도를 최적화합니다."""
    repo_paths = []
    if not os.path.isdir(base_path):
        print(f"❌ 경고: 기준 디렉토리를 찾을 수 없습니다: {base_path}")
        return repo_paths
        
    # 탐색에서 제외할 무거운 의존성 폴더 목록
    SKIP_DIRS = { 'node_modules', 'vendor', '.venv', 'venv', 'target', 'build' }
        
    for root, dirs, files in os.walk(base_path):
        # 💡 1. 무거운 폴더들은 os.walk 탐색 대상에서 즉시 제외 (속도 향상 핵심)
        dirs[:] = [d for d in dirs if d not in SKIP_DIRS]
        
        if '.git' in dirs:
            repo_paths.append(root)
            # 💡 2. .git을 이미 찾은 폴더 내부의 하위 폴더는 더 이상 샅샅이 뒤지지 않음
            dirs.clear() 
